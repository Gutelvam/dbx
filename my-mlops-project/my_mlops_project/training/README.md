...
directory of the corresponding component, for example, model training notebooks in `my_mlops_project/training/notebooks`, 
batch inference notebook in `my_mlops_project/deployment/batch_inference/notebooks`.
...
Address TODOs in the recipe configs under `my_mlops_project/training/recipe.yaml`, `my_mlops_project/training/profiles/databricks-dev.yaml`,
and `my_mlops_project/training/profiles/local.yaml`, specifying configs such as the training dataset path(s) to use when developing
...
After configuring your recipe, you can iterate on and test ML code under ``my_mlops_project/training/steps``.
We expect most development to take place in the abovementioned YAML config files and
`my_mlops_project/training/steps/train.py` (model training logic).
...
You can iterate on ML code by running the provided `my_mlops_project/training/notebooks/TrainWithMLflowRecipes.py` notebook on Databricks using
[Repos](https://docs.databricks.com/repos/index.html). This notebook drives execution of
the ML code defined under ``my_mlops_project/training/steps``. You can use multiple browser tabs to edit
...
#### Prerequisites
* Python 3.8+
* Install model training and test dependencies via `pip install -I -r my_mlops_project/requirements.txt -r test-requirements.txt` from project root directory.
